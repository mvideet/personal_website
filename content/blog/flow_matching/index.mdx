...
1. Replace the discrete time steps with a continuous time parameter $t \in [0,1]$
2. Express $\alpha_t$ as a continuous function: $\alpha_t = e^{-\frac{1}{2}T(t)}$ 
3. Replace the discrete noise schedule $\beta_t$ with a continuous function $\beta(t)$
4. Define $T(t)$ as the cumulative integral: $T(t) = \int_0^t \beta(s)ds$
